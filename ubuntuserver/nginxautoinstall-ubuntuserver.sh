...
NGINX_VERSION="1.7.12"   # The mainline version
...
displayandexec "Install Redis" $APT_GET install redis-server
...
if [ $TAGINSTALL == 1 ]
then
  displayandexec "Init the default configuration file for NGinx" "$WGET https://raw.github.com/P3ter/Serveur/master/ubuntuserver/nginx/nginx.conf ; $WGET https://raw.github.com/P3ter/Serveur/master/ubuntuserver/nginx/default ; mv nginx.conf /etc/nginx/ ; mv default /etc/nginx/sites-enabled/"
fi

# Download the init script
displayandexec "Install the NGinx init script" "$WGET https://raw.github.com/P3ter/Serveur/master/ubuntuserver/nginx/nginx ; mv nginx /etc/init.d/ ; chmod 755 /etc/init.d/nginx ; /usr/sbin/update-rc.d -f nginx defaults"
